    ...
    <body class="transition-all duration-500 ease-in-out flex flex-col justify-start items-start gap-6 w-screen h-screen bg-black">
        ...
        @if(session('message'))
            <div id="session" class="transition-all duration-500 ease-in-out fixed top-0 left-1/3 text-white w-80 h-40 flex justify-center items-center z-10 border-2 border-white">
                <span class="transition-all duration-500 ease-in-out bg-black w-10/12 flex justify-center items-center flex-wrap text-center font-extrabold text-xl rounded-xl p-2">{{ session('message') }}</span>
        ...
        <div id="contain-search-bar" class="transition-all duration-500 ease-in-out relative w-screen flex justify-start items-center flex-wrap gap-2">
            <form action="{{ route('admin.user.index') }}" method="GET" class="transition-all duration-500 ease-in-out w-80">
                @csrf
        ...
        <table class="transition-all duration-500 ease-in-out w-full text-center text-white border-2 border-white">
            ...
            @forelse ($datas as $data)
                <tr>
                    <td class="transition-all duration-500 ease-in-out text-xl text-center border-2 border-white">{{ $data->id }}</td>
                    <td class="transition-all duration-500 ease-in-out text-xl text-center border-2 border-white">{{ $data->nom }}</td>
                    <td class="transition-all duration-500 ease-in-out text-xl text-center border-2 border-white">{{ $data->email }}</td>
                    <td class="transition-all duration-500 ease-in-out text-xl text-center border-2 border-white"><a href="{{ route('admin.user.show', $data->id) }}" class="fa fa-eye text-2xl text-lime-200 hover:cursor-pointer"></a></td>
